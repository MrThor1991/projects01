...
<?php
	...
	if (isset($_POST["btn_submit"])) {
		...
		//làm sạch thông tin, xóa bỏ các trang html, ký tự đặc biệt
		//mà người dùng cố tình thêm vào để tấn công theo phương thức sql ịnection
		$username = strip_tags ($username);
		$username = addslashes($username);
		$password = strip_tags ($password);
		$password = addslashes($password);
		if ($username == "" || $password ==""){
			echo "username và password bạn ko đc để trống!";
		}else{
